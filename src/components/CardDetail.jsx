...
export default function CardDetail({ url, title, content, liveAt = '', onClose, credits = [] }) {
  ...
  return (
    <div className="relative w-full max-w-[900px] h-full sm:h-[600px] sm:pb-3 bg-black/80 rounded-xl">
      <div className="absolute right-0 p-3">
        <button
          className="bg-zinc-500/50 p-1 rounded"
          onClick={onClose}
        >
          <CloseIcon />
        </button>
      ...
      <article className='flex flex-col px-[50px] pt-[30px] justify-start items-center w-full h-full'>
              ...
              credits.map(item => (
                <li key={item.author} className="mb-2">
                  <p className="text-sm m-0">
                    <a href={item.contact} className="text-sm">{item.author}</a>: {item.work}
                  </p>
                </li>
              ))
            }
        ...
        { liveAt && <p>Live at: <a href={liveAt} target="blank">{liveAt}</a></p> }
        { url && loading && <div className="relative top-[170px]"><SpinnerIcon /></div>}
        <iframe
